...
class preprocessing_life:
    ...
    def create_binarized_df(self, X_miss, cat_cols, num_cols, spec_cols):
        df_num_bins = self.discretize_num(X_miss, num_cols) # discretize numeric features with missingness
        df_cat_dummies = self.get_cat_dummies(X_miss, cat_cols)  #Onehotencode categorical features 
        df_spec = pd.DataFrame(X_miss[spec_cols])
        ...
        X_m = pd.concat([df_num_bins, df_spec, df_cat_dummies], axis=1, ignore_index=True)
