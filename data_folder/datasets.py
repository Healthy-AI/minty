...
class Create_data:
    ...
    def get_synth(self):
        X_m, Y, M = generate_data(n=7000, c=15, delta=0.1, gamma=0.1) #new missingness generation process
        columns = ['X' + str(i) for i in range(1, X_m.shape[1] + 1)]
        X_df = pd.DataFrame(X_m, columns=columns)
        
        return X_m, Y, X_df

    def get_ADNI(self, estimator=None):
        if estimator == 'minty':
    ...
    def get_housing(self, estimator=None):
        X_miss, Y, cat_cols, num_cols, num_special = preprocessing_housing().get_data()
        if estimator == 'minty':
            X_m = preprocessing_housing().create_binarized_df(X_miss, cat_cols, num_cols, num_special) #binarize categoricals and split numericals
        else:
    ...
    def get_life(self, estimator=None):
        ...
        if estimator == 'minty':
            ...
            df_cat_dummies = preprocessing_life().get_cat_dummies(X_miss, cat_cols)  # encode categoricals in missingness fram
            X_miss_no_original_categorical= X_miss.drop(columns=cat_cols).copy()  # remove orginial cat_columns form df with missingness
            X_m = pd.concat([X_miss_no_original_categorical, df_cat_dummies], axis=1)  # concatenate encoded categoricals to df with missingness
